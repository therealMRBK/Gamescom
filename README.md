...
- **Hallenplan** (🗺️) – interaktive Routensuche direkt auf dem offiziellen gamescom-2026-Hallenplan (Bild unter `public/hallplan/`): Marken für alle Hallen/Eingänge sind auf die echten Positionen im Plan kalibriert, die Route zwischen zwei Punkten wird darauf eingezeichnet. Die Nachbarschaften (welche Halle grenzt an welche) sind aus dem Plan abgelesen, aber es ist kein exaktes Innenraum-Wegeleitsystem (Gänge, Treppen, Aufzüge).
